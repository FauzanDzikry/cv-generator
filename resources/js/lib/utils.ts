...
export function convertOklchToRgb(colorStr: string): string {
    ...
    return colorStr.replace(
        ...
        (match, lStr, cStr, hStr, aStr) => {
            ...
            const bLinear = -0.0041960863 * lCube - 0.7034186147 * mCube + 1.7076147010 * sCube;
...
export function sanitizeOklchColors(clonedDoc: Document, clonedEl?: HTMLElement): void {
    ...
    const elements = clonedEl
        ? ([clonedDoc.documentElement, clonedDoc.body, clonedEl, ...Array.from(clonedEl.querySelectorAll('*'))].filter(
              Boolean,
          ) as HTMLElement[])
